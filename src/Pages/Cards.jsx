...
function Cards() {
    ...
    const fetchCards = async () => {
      try {
        ...
        if (response.data.length > 0) {
          setSelectedCardId(response.data[0].id); // Select first card by default
        }
      } catch (error) {
  ...
  // Fetch IBAN + card info based on selectedCardId
  useEffect(() => {
  ...
  return (
    <>
      ...
      {/* IBAN et infos carte dynamiques */}
      <div className="flex flex-col justify-center px-3 py-2 space-y-4 bg-white rounded-md md:flex-row md:space-y-0 md:space-x-24 md:items-center">
        <div className="flex items-center space-x-4">
          <button className="px-2 py-1 font-mono text-gray-600 border border-gray-500 rounded-xl hover:bg-gray-100">
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-400">Status: {card.status ? "Active" : "Inactive"}</p>
                </div>
