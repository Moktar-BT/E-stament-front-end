...
function CreditCard() {
  ...
  useEffect(() => {
    const fetchCreditCards = async () => {
      try {
        const response = await fetch('http://localhost:8083/Dashboard/getCreditCardSummary', {
          headers: {
        ...
        const transformedData = apiData.map(card => {
          ...
          const utilizationPercentage = (card.currentBalance / card.availableLimit) * 100;
          ...
          const formattedNumber = card.cardNumber.replace(/(\d{4})(\d{4})(\d{4})(\d{4})/, '$1 - $2 - $3 - $4');
  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-between w-full p-4 bg-white border border-gray-100 rounded-lg shadow-sm" style={{ minHeight: "397px" }}>
